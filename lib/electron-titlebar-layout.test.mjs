...
test("macOS Electron title bar shows the sidebar title right of the traffic lights", () => {
  ...
  // The traffic lights (x: 14-66) overlap the header row, so the title is
  // shifted right of them (10px row padding + 70px margin => x=80) instead of
  // being hidden.
  assert.match(
    globalStyles,
    /html\[data-electron-platform="darwin"\]\s+\.sidebar-title-row\s+\.pi-agent-title\s*\{[^}]*margin-left:\s*70px/s,
  );
  assert.doesNotMatch(
    globalStyles,
